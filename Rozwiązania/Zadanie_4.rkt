...
(define-type Value
  (numV [n : Number])
  (boolV [b : Boolean])
  (funV [x : Symbol] [e : Exp])     ; Usuwamy pole przechowujące środowisko
  (primopV [f : (Value -> Value)])) ; w konstruktorze funV
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(lamE x b)
     (funV x b)] ; Usuwamy środowisko z kontruktora funV
    [(appE e1 e2)
     (apply (eval e1 env) (eval e2 env) env)])) ; Jako trzeci argument,
                                                ; funkcja apply przyjmie środowisko

(define (apply [v1 : Value] [v2 : Value] [env : Env]) : Value 
  (type-case Value v1
    [(funV x b)                      ; W przypadku napotkania konstruktora funV
     (eval b (extend-env env x v2))] ; wywołujemy funkcję eval z argumentem b
    [(primopV f)                     ; oraz środowiskiem zawierającym zmienną x
     (f v2)]                         ; z przypisaną wartością
  ...
  (test (run `{lambda {x} {lambda {y} {+ x y}}})
        (funV 'x
              (lamE 'y (appE (appE (varE '+)
                                      (varE 'x))
                                (varE 'y))))) ; Modyfikujemy test,
  ; aby zwracał poprawną odpowiedź dla nowej implementacji funkcji eval i apply
  (test (run `{let a 1
                {let p {lambda {x} {+ x a}}
                  {let a 10
                    {p 0}}}})
        (numV 10))) ; Modyfikujemy test, aby zwracał poprawną odpowiedź
                    ; dla nowej implementacji funkcji eval i apply
...
(define (value->string [v : Value]) : String
  (type-case Value v
    ...
    [(funV x e) "#<procedure>"] ; Usuwamy środowisko z konstruktora funV
    [(primopV f) "#<primop>"]))
