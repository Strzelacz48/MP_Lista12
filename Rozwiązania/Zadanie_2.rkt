...
(define-type Value
  ...
  (funAV [e : ExpA] [env : (EnvA 'a)])) ; Wprowadzamy nowy konstruktor reprezentujący
                                        ; funkcję w postaci ExpA
...
(define (evalA [a : ExpA] [env : (EnvA Value)]) : Value
  (type-case ExpA a
    ...
    [(lambdaA e)                               ; W przypadku wystąpienia lambdy
     (funAV e env)]                            ; wrzucamy ją w konstruktor (funAV)
    [(appA e1 e2)                              ; W przypadku wystąpienia aplikacji
     (applyA (evalA e1 env) (evalA e2 env))])) ; wywołujemy funckję applyA z wyliczonymi
                                               ; argumentami e1 i e2

(define (applyA [v1 : Value] [v2 : Value]) : Value ; Funckja applyA robi to samo co apply,
  (type-case Value v1                              ; tyle, że dla wyrażeń z języka ExpA
    [(funAV e env)
...
(define (translate [e : Exp] [env : (EnvA Symbol)]) : ExpA
  (type-case Exp e
    ...
    [(lamE x e)                                      ; Translacja lambd wygląda tak samo
     (lambdaA (translate e (extend-envA env x)))]    ; jak translacja letów
    [(appE e1 e2)                                    ; Aplikacje translatujemy odwołując
     (appA (translate e1 env) (translate e2 env))])) ; się rekurencyjnie do jej argumentów
